...
export class AIstorage implements IAIstorage {
    ...
    private async getInteractionRecord(): Promise<{
        ...
        const [result] = (await this.persistenceRead.readByAssociation(
            association,
        )) as Array<{ prompt: string; response: string; message: string , messageHistory : Array<{ text: string; sender: string }> }>;
        return result || { prompt: '', response: '', message: '', messageHistory: [] };
    }
    ...
    public async updateMessageHistory(
        ...
        const record = await this.getInteractionRecord();
        record.messageHistory = messageHistory;
        await this.storeInteractionRecord(record);
    }

    public async getMessageHistory(): Promise<Array<{ text: string; sender: string }>> {
        const record = await this.getInteractionRecord();
        return record.messageHistory;
    }
}
