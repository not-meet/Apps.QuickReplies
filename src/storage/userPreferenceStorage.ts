...
export class UserPreferenceStorage implements IuserPreferenceStorage {
    ...
    public async setListeningState(isListening: boolean): Promise<void> {
        const association = new RocketChatAssociationRecord(
            RocketChatAssociationModel.USER,
            `${this.userId}#preference`,
        );
        ...
        preference.isListening = isListening;

        await this.persistence.updateByAssociation(association, { preference }, true);
    }
