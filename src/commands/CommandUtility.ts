...
export class CommandUtility implements ICommandUtility {
    ...
    public async resolveCommand(): Promise<void> {
        ...
        if (this.params.length && this.params.length > 1) {
            ...
            if (subCommand === CommandParam.GRAMMER) {
                await this.handleSingleParam(handler)
                return;
            }
        }
    ...
    private async handleSingleParam(handler: Handler): Promise<void> {
        switch (this.params[0].toLowerCase()) {
            ...
            case CommandParam.AI:
                ...
                break;
            case CommandParam.GRAMMER:
                await handler.CorrectGrammer();
                console.log('CorrectGrammer called')
                break;
            case CommandParam.BOTON:
                await handler.StartListning();
                console.log('bot has been called---- activated')
                break;
            case CommandParam.BOTOFF:
                await handler.StopListning();
